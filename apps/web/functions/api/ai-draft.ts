...
function parseDraft(content: string): Draft | null {
  try {
    ...
    return {
      ...
      needsConfirmation,
      questionsForLawyer,
      notice: notice || '이 초안은 사용자가 입력한 내용을 정리한 참고용 정보이며, 법률 판단이나 대응 전략이 아닙니다.',
    }
  } catch {
    return null
  }
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  if (!env.LLM_API_KEY || !env.LLM_API_URL || !env.LLM_MODEL) {
    return json({ error: 'AI 초안 기능이 아직 설정되지 않았습니다.' }, 503)
  }
  ...
  if (!incidentText || incidentText.length > MAX_INCIDENT_LENGTH) {
    return json({ error: `사건 내용은 1~${MAX_INCIDENT_LENGTH}자로 입력해 주세요.` }, 400)
  }
...
{"userFacts":["사용자가 직접 말한 사실"],"needsConfirmation":["추가 확인이 필요한 정보"],"questionsForLawyer":["변호사에게 물어볼 중립적인 질문"],"notice":"법률 판단이 아닌 참고용 초안이라는 짧은 고지"}`
  ...
  try {
    ...
    if (!upstream.ok) {
      console.error('LLM request failed', upstream.status)
